...
const FindTeamMates: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDefaultProfiles = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/user/findUsers`, {
          credentials: "include",
        });
        if (response.ok) {
  ...
  return (
    <div className="w-full min-h-screen bg-black px-4 md:px-[250px]">
      ...
      <div className="w-full h-[10%] md:mt-[20px] mt-[10px] mb-3 p-3 relative">
        <div className="w-full h-full border-4 border-zinc-100 flex rounded-xl">
          <div className="h-full pt-2 w-[10%] md:w-[5%] flex justify-center items-center">
